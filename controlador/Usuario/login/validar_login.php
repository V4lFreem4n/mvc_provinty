...
if (!$db) {
    ...
    header("Location: ../../../public/login-trabajadores.php?error=" . urlencode($error));
    //header("Location: A");
    exit();
}
...
if (empty($email) || empty($password)) {
    ...
    $error =  "Por favor, complete todos los campos.";; // Definir el mensaje de error
    header("Location: ../../../public/login-trabajadores.php?error=" . urlencode($error));
    //header("Location: B");
    exit();
...
if ($resultado && $resultado->num_rows === 1) { 
    ...
    if ($user['activo'] != 1) {
        $error = "Esta cuenta está desactivada. Por favor contacte al administrador.";
        header("Location: ../../../public/login-trabajadores.php?error=" . urlencode($error));
        //header("Location: C");
        exit();
    }
